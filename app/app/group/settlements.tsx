// app/app/group/settlements.tsx
//
// Les remboursements ont leur propre ecran : sur l'ecran de groupe ils
// occupaient la moitie de la page et repoussaient les depenses hors de vue,
// alors qu'on ne les consulte pas a chaque ouverture.
//
// La logique est reprise telle quelle depuis l'ecran de groupe — meme cle de
// requete, donc meme cache react-query et aucune requete supplementaire.
...
import { groupsApi, expensesApi, settlementsApi } from '../../src/services/api';
import { useAuthStore } from '../../src/store/authStore';
import { useFormatMoney, useCurrency, useT } from '../../src/store/langStore';
import { Avatar, Card, SectionLabel, Divider } from '../../src/components/ui';
import Feather from '@expo/vector-icons/Feather';
import { colors, spacing, radius, fonts, money } from '../../src/theme';
import { Balance, Settlement } from '../../../shared/types';
...
export default function SettlementsScreen() {
  ...
  if (isLoading || !group) return <View style={styles.screen} />;

  const myMember = group.members.find((m: any) => m.userId === user?.id);

  // Log par depense : le detail depliable sous chaque solde.
    ...
    <View style={styles.screen}>
      <View style={[styles.header, { paddingTop: Math.max(insets.top, 20) + 8 }]}>
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          <Text style={styles.title} numberOfLines={1}>{t('settlements.section')}</Text>
          <Text style={styles.subtitle} numberOfLines={1}>{group.emoji} {group.name}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: fonts.semibold, fontSize: 20, color: colors.text, letterSpacing: -0.4 },
  subtitle: { fontFamily: fonts.regular, fontSize: 13, color: colors.text3, marginTop: 2 },
  scroll: { paddingHorizontal: spacing.xl, paddingBottom: 60, paddingTop: 8 },
